...
class Patient:
    ...
    #saves patient details to the records
        patient_data = f"{self.__first_name} | {self.__surname} | {self.__age} | {self.__mobile} | {self.__postcode} | {','.join(self.__symptoms)}\n"
        ...
        try:
            with open("patients.txt", 'w') as fd:  # Use forward slash (/) for the file path
                ...
                for index, c in enumerate(lst):
                    fd.write(f"{index+1:3}|{c.full_name():^30}|{c.get_doctor():^30}|{c._Patient__age:^5}|{c._Patient__mobile:^15}|{c._Patient__address:^10}|{', '.join(c._Patient__symptoms):^20} \n")  # Access attributes using the correct name and convert the symptoms list to a comma-separated string
        except:
            return False
        else:
            return True   
        ...
        try:
            with open("HMS/patients.txt", 'r') as fd:
                ...
                for line in fd:
                    ...
                    patient = Patient(full_name, age, mobile, address, ','.join(symptoms))
